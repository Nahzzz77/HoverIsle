<div align="center">
  <img src="build/hoverisle-icon.png" width="112" alt="悬岛 HoverIsle 图标" />
  <h1>悬岛 · HoverIsle</h1>
  <p><strong>把 Mac 屏幕顶部，变成随手可用的工作岛。</strong></p>
  <p>待办、随笔记、链接、录音与本机 AI 提醒，始终贴顶待命。</p>
  <p>
    <a href="https://github.com/Nahzzz77/HoverIsle/releases/latest"><strong>下载 macOS 版</strong></a>
    ·
    <a href="#从源码运行">从源码运行</a>
    ·
    <a href="#更新日志">更新日志</a>
    ·
    <a href="https://github.com/Nahzzz77/HoverIsle/issues">反馈问题</a>
  </p>
  <p>
    <img alt="Release" src="https://img.shields.io/github/v/release/Nahzzz77/HoverIsle?style=flat-square&color=7c8cff" />
...
![悬岛首页](docs/screenshots/home.png)

![悬岛待办](docs/screenshots/todo.png)

## 它是什么

悬岛（HoverIsle）是一个常驻 macOS 屏幕顶部的本地工作台。默认折叠成物理刘海大小，点击后从顶部展开；常用信息和动作不必再散落在多个应用里。

悬岛（HoverIsle）由 `Nahzzz77` 维护，产品界面和公开品牌均使用悬岛名称。
...
>
> 项目仓库：[Nahzzz77/HoverIsle](https://github.com/Nahzzz77/HoverIsle)

1. 当前分支尚未发布自己的公开 Release；先按下方“从源码运行”构建本机安装包。
2. 打开生成的 DMG，将 `悬岛.app` 拖入「应用程序」。
...
当前版本选择“迁移兼容”。为避免旧版本升级后丢失内容，悬岛继续使用历史默认目录 `~/Library/Application Support/Dynamic Panel`，并继续读取其中 `workspace-settings.json` 指向的自定义工作区。会保留的内容包括：

- `workspace.json`：待办、笔记、链接、录音元数据、剪贴板历史和首页布局等本地工作区快照；
- `recordings/` 与 `clipboard-images/`：录音文件和剪贴板图片；
- `mirror-cover.jpg`：镜子背景图；
- `app-settings.json` 与 `workspace-settings.json`：功能开关、快捷键和自定义数据目录；
- `credentials.vault.json` 与 `transcription-settings.json`：由 macOS `safeStorage` 保护的账号/密码与转写、AI 配置。

早期版本遗留的 `~/Library/Application Support/notch-todo/transcription-settings.json` 也会自动读取并迁移。内部 `notch-*` 存储键同样刻意保留，这是兼容层，不是对外显示的产品名称。本机目前没有发现默认旧目录，所以这台 Mac 的首次运行会是空工作区；如果你以前选过其他数据文件夹，悬岛会继续使用那个文件夹。
...
悬岛只在 `127.0.0.1:43821` 监听通知接口，来源限 `codex`、`claude` 与 `gpt`：
...
```text
/Applications/悬岛.app/Contents/Resources/app/scripts/codex-notify.js
/Applications/悬岛.app/Contents/Resources/app/scripts/claude-notify.js
```
...
[MIT](LICENSE) © 2026 `Nahzzz77`；原项目来源与版权说明见 [NOTICE.md](NOTICE.md)。
