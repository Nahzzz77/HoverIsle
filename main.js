...
const APP_NAME = '悬岛';
const APP_ENGLISH_NAME = 'HoverIsle';
// Public repository for the HoverIsle product and its release links.
const PROJECT_REPOSITORY_URL = 'https://github.com/Nahzzz77/HoverIsle';
    ...
    { type: 'separator' },
    {
      ...
      click: () => {
        dialog.showMessageBox({
          type: 'info',
          title: `关于 ${APP_NAME}`,
          message: `${APP_NAME} / ${APP_ENGLISH_NAME}`,
          detail:
          ...
          if (response === 0) shell.openExternal(PROJECT_REPOSITORY_URL);
        });
      },
    },
    { type: 'separator' },
...
function createTray() {
  ...
  tray.setToolTip(`${APP_NAME} / ${APP_ENGLISH_NAME}`);
  tray.on('click', () => {
...
async function promptForMissingPermissions() {
  ...
  const { response, checkboxChecked } = await dialog.showMessageBox({
    type: 'info',
    message: `${APP_NAME} 需要「${names.join('」和「')}」权限`,
    detail: [
      '缺少这些权限时，「当前窗口」会读不到任何窗口，汽水音乐的播放控制也不会生效。',
      '',
      `授权后需要重新启动 ${APP_NAME} 才会生效。`,
  ...
  try {
    const response = await fetch(safeEndpoint, {
      ...
      headers: {
        ...
        'User-Agent': `${APP_ENGLISH_NAME}/1.0 (+local bookmark organizer)`,
      },
      body: JSON.stringify({
        model: config.model,
        temperature: 0.1,
    ...
    try {
      response = await fetch(current, {
        ...
        headers: {
          ...
          'User-Agent': `${APP_ENGLISH_NAME}/1.0 (+local bookmark metadata)`,
        },
      });
    } catch (error) {
      clearTimeout(timeout);
...
async function rememberPasteTarget() {
  ...
  if (current && !['com.github.Electron', 'com.vibecoding.notch-todo', 'com.dynamicpanel.app', 'com.nahzzz77.hoverisle'].includes(current.bundleId)) {
...
ipcMain.handle('transcription:start', (event) => {
  ...
  return new Promise((resolve) => {
    const headers = {
      ...
      'User-Agent': `${APP_ENGLISH_NAME}/1.0`,
    };
